...
pub fn parse_typescript_file(path: &std::path::Path, content: &str) -> Result<ParsedFile> {
    ...
    parser
        ...
        .map_err(|e| FunveilError::TreeSitterError(format!("Failed to load TypeScript parser: {e}")))?;

    let tree = parser
        .parse(content, None)
        .ok_or_else(|| FunveilError::TreeSitterError("Failed to parse TypeScript file".to_string()))?;
...
fn extract_react_components(tree: &Tree, content: &str) -> Result<Vec<Symbol>> {
    ...
    let arrow_query = Query::new(&tsx_lang, TS_ARROW_COMPONENT_QUERY)
        .map_err(|e| FunveilError::TreeSitterError(format!("Invalid TS arrow component query: {e}")))?;
